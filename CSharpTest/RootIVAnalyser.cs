...
public class RootIVAnalyser : IDisposable
{
    ...
    public static void CalculateBreakdown(string file, out double Vbr, out double compVbr, out double chi2, bool useGradientCompensation = true)
    {
        CurrentMeasurementDataModel c = JSONHelper.ReadJsonFile<CurrentMeasurementDataModel>(file);
        CalculateBreakdown(c, out Vbr, out compVbr, out chi2, useGradientCompensation);
    }

    public static void CalculateBreakdown(CurrentMeasurementDataModel c, out double Vbr, out double compVbr, out double chi2, bool useGradientCompensation = true)
    {
        Console.WriteLine("Starting analysis...");
        RootIVAnalyser iv = new RootIVAnalyser();
        ...
        List<double> temperatures = GetTemperatures(c, useGradientCompensation);
